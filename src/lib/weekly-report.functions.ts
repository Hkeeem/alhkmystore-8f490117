...
export const listReportSettings = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      context,
    }): Promise<{ recipients: Array<ReportRecipient>; runs: Array<ReportRun> }> => {
      ...
      const [{ data: recipients }, { data: runs }] = await Promise.all([
        ...
        supabaseAdmin
          .from("report_runs")
          .select(
            "id, period_start, period_end, days, status, recipients, error, triggered_by, created_at",
          )
...
export const sendReportNow = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: { days?: number; testEmail?: string }) => {
    ...
    const testEmail = String(data?.testEmail ?? "")
      .trim()
      .toLowerCase();
