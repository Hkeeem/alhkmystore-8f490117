...
import {
  createPropertyListingAndMatchBuyers,
  submitBuyerRequest,
} from "@/lib/reverse-property-matchmaking.functions";
...
export const Route = createFileRoute("/real-estate")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "مطابقة عقارية ذكية تجمع الباحثين والمعلنين وفق الموقع والميزانية والمميزات.",
      },
    ],
  }),
...
function LocationFields({
  ...
  const districts = useMemo(
    () => getSaudiDistricts(value.region, value.city),
  ...
  return (
    <section aria-labelledby="seeker-form-title" className="space-y-4">
      <div className="rounded-3xl border border-primary/20 bg-primary/5 p-4">
        <h2
          id="seeker-form-title"
          className="flex items-center gap-2 text-lg font-black text-foreground"
        >
          <Search className="h-5 w-5 text-primary" /> سجّل طلبك العقاري
        </h2>
        <p className="mt-1 text-sm leading-6 text-muted-foreground">
          حدّد ما تبحث عنه، وسنستخدم المعايير نفسها لمطابقة المعلنين بعقارك عند وصوله.
        </p>
      </div>

      {!user && (
        <div className="rounded-3xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-900 dark:text-amber-200">
          تحتاج إلى حساب لحفظ الطلب وإدارة بيانات التواصل.{" "}
          <Link to="/auth" className="font-black underline">
            تسجيل الدخول
          </Link>
        </div>
      )}
      ...
      <div className="grid gap-3 rounded-3xl border border-border/60 bg-card p-4 shadow-card sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            className="h-12 w-full rounded-2xl border border-border bg-secondary px-3 text-sm font-bold outline-none focus:border-primary"
          />
        </div>
        <div>
          ...
          <input
            ...
            className="h-12 w-full rounded-2xl border border-border bg-secondary px-3 text-right text-sm font-bold outline-none focus:border-primary"
          />
        </div>
      </div>

      <PurposeAndTypeFields
        purpose={form.purpose}
        propertyType={form.propertyType}
        onPurposeChange={(purpose) => setForm((current) => ({ ...current, purpose }))}
        onTypeChange={(propertyType) => setForm((current) => ({ ...current, propertyType }))}
      />
      <LocationFields value={form} onChange={updateLocation} />

      <div className="grid gap-3 rounded-3xl border border-border/60 bg-card p-4 shadow-card sm:grid-cols-2">
        <div>
          <FieldLabel icon={<WalletCards className="h-4 w-4 text-primary" />}>
            الميزانية القصوى (ر.س)
          </FieldLabel>
          <input
            type="number"
            min="1"
        ...
        <div>
          <FieldLabel icon={<BedDouble className="h-4 w-4 text-primary" />}>
            الحد الأدنى للغرف
          </FieldLabel>
          <input
            type="number"
      ...
      <ChoiceChips
        ...
        onToggle={(value) =>
          setForm((current) => ({ ...current, features: toggleSelection(current.features, value) }))
        }
      />
      <ChoiceChips
        ...
        onToggle={(value) =>
          setForm((current) => ({ ...current, services: toggleSelection(current.services, value) }))
        }
      />
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-3xl border border-border/60 bg-card p-4 text-sm leading-6 text-muted-foreground shadow-card">
        ...
        />
        <span>
          <strong className="text-foreground">أوافق على التواصل بشأن طلب البحث.</strong> لن يُعرض
          رقمي إلا لمعلنين لديهم عقار متوافق ومن خلال نتيجة المطابقة.
        </span>
      </label>
      ...
      <button
        ...
        className="flex h-14 w-full items-center justify-center gap-2 rounded-3xl bg-primary text-base font-black text-primary-foreground shadow-glow transition-transform active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60"
      >
...
function MatchBadge({ score }: { score: number }) {
  ...
  return (
    <span className={`inline-flex rounded-full px-3 py-1 text-sm font-black ${className}`}>
      {score}% تطابق
    </span>
  );
}
    ...
    <article className="rounded-3xl border border-border/60 bg-card p-4 shadow-card">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {match.is_demo && (
              <span className="rounded-full bg-sky-500/10 px-2 py-1 text-[11px] font-black text-sky-700 dark:text-sky-300">
                بيانات تجريبية
              </span>
            )}
          </div>
          <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
            <MapPin className="h-3.5 w-3.5" /> {match.district}، {match.city}
          </p>
        </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
        <div className="rounded-2xl bg-secondary p-3">
          <span className="block text-xs text-muted-foreground">الميزانية القصوى</span>
          <strong>{Number(match.max_price).toLocaleString("ar-SA")} ر.س</strong>
        </div>
        <div className="rounded-2xl bg-secondary p-3">
          <span className="block text-xs text-muted-foreground">العقار المطلوب</span>
          <strong>
            {match.property_type} · {match.min_bedrooms}+ غرف
          </strong>
        </div>
      </div>
      ...
      {(match.features.length > 0 || match.required_services.length > 0) && (
        <p className="mt-3 text-xs leading-5 text-muted-foreground">
          المميزات والخدمات:{" "}
          {[...match.features, ...match.required_services].slice(0, 4).join("، ")}
        </p>
      )}
...
function AdvertiserPanel() {
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        resolvedMatches.length
          ? `وجدنا ${resolvedMatches.length} مشتريًا متوافقًا.`
          : "لا توجد طلبات متوافقة حاليًا.",
      );
    },
    onError: (error) =>
  ...
  const updateLocation = (location: LocationValue) =>
    setProperty((current) => ({ ...current, ...location }));
  const handleMatch = () => {
    if (!user) {
  ...
  return (
    <section aria-labelledby="advertiser-form-title" className="space-y-4">
      <div className="rounded-3xl border border-primary/20 bg-primary/5 p-4">
        <h2
          id="advertiser-form-title"
          className="flex items-center gap-2 text-lg font-black text-foreground"
        >
          <Building2 className="h-5 w-5 text-primary" /> إضافة عقار للمطابقة
        </h2>
        <p className="mt-1 text-sm leading-6 text-muted-foreground">
          أدخل تفاصيل العقار، ثم اعرض الباحثين المتوافقين بترتيب نسبة التطابق.
        </p>
      </div>

      {!user && (
        <div className="rounded-3xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-900 dark:text-amber-200">
          تحتاج إلى حساب لاستخدام ميزة المطابقة.{" "}
          <Link to="/auth" className="font-black underline">
            تسجيل الدخول
          </Link>
        </div>
      ...
      <PurposeAndTypeFields
        purpose={property.purpose}
        propertyType={property.propertyType}
        onPurposeChange={(purpose) => setProperty((current) => ({ ...current, purpose }))}
        onTypeChange={(propertyType) => setProperty((current) => ({ ...current, propertyType }))}
      />
      ...
      <div className="grid gap-3 rounded-3xl border border-border/60 bg-card p-4 shadow-card sm:grid-cols-2">
        <div>
          ...
          <input
            ...
            onChange={(event) =>
              setProperty((current) => ({ ...current, price: event.target.value }))
            }
        ...
        <div>
          ...
          <input
            ...
            onChange={(event) =>
              setProperty((current) => ({ ...current, bedrooms: event.target.value }))
            }
            className="h-12 w-full rounded-2xl border border-border bg-secondary px-3 text-sm font-bold outline-none focus:border-primary"
          />
        </div>
      ...
      <ChoiceChips
        ...
        onToggle={(value) =>
          setProperty((current) => ({
            ...current,
            features: toggleSelection(current.features, value),
          }))
        }
      />
      <ChoiceChips
        label="الخدمات القريبة والمتاحة"
        items={SERVICES}
        selected={property.services}
        onToggle={(value) =>
          setProperty((current) => ({
            ...current,
            services: toggleSelection(current.services, value),
          }))
        }
      />
      ...
      {(matches.length > 0 || matchMutation.isSuccess) && (
        <section
          aria-live="polite"
          className="space-y-3 rounded-[2rem] border border-emerald-500/25 bg-emerald-500/[0.04] p-4"
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {matches.length
                  ? "رتّبنا الطلبات وفق الموقع والميزانية والغرف والمميزات والخدمات."
                  : "لا توجد طلبات تتجاوز حد المطابقة الحالي. جرّب تعديل التفاصيل لاحقًا."}
              </p>
            </div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            {matches.map((match) => (
              <BuyerMatchCard key={match.id} match={match} property={property} />
            ))}
          </div>
        </section>
      )}
    ...
    <main dir="rtl" className="mx-auto max-w-3xl space-y-6 px-4 pb-20 pt-6">
      <section className="relative overflow-hidden rounded-[2rem] bg-gradient-hero p-6 text-primary-foreground shadow-glow">
        ...
        <div className="relative flex items-start gap-4">
          ...
          <div>
            <div className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-white/15 px-2.5 py-1 text-xs font-bold">
              <Sparkles className="h-3 w-3" /> ذكاء اصطناعي
            </div>
            <h1 className="font-display text-2xl font-black">البحث العقاري الذكي</h1>
            <p className="mt-1 text-sm text-white/75">
              نطابق طلبات الباحثين مع عقارات المعلنين وفق معايير واضحة وقابلة للتخصيص.
            </p>
          </div>
        </div>
      </section>
      ...
      <nav
        aria-label="نوع الخدمة العقارية"
        className="grid grid-cols-2 gap-2 rounded-3xl border border-border/60 bg-card p-2 shadow-card"
      >
        <button
          type="button"
          onClick={() => setMode("seeker")}
          className={`flex h-12 items-center justify-center gap-2 rounded-2xl text-sm font-black transition-colors ${mode === "seeker" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"}`}
        >
          <Search className="h-4 w-4" /> أنا أبحث عن عقار
        </button>
        <button
          type="button"
          onClick={() => setMode("advertiser")}
          className={`flex h-12 items-center justify-center gap-2 rounded-2xl text-sm font-black transition-colors ${mode === "advertiser" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary"}`}
        >
          <Building2 className="h-4 w-4" /> أنا أعرض عقار
        </button>
      </nav>
      ...
      <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
        ...
        متطابقة ومصرّح بالتواصل معها.
      </div>
      <Link to="/" className="mx-auto flex w-fit items-center gap-1 text-sm font-bold text-primary">
