...
type LiveData = { prices: LivePrice[]; coupons: ActiveCoupon[] };

// ─── هوك لجلب البيانات الحية مع منع تسريب الذاكرة ────────
function useLiveData(): { data: LiveData; loading: boolean } {
  const [data, setData] = useState<LiveData>({ prices: [], coupons: [] });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setData({
          prices: (prices as LivePrice[]) ?? [],
          coupons: (coupons as ActiveCoupon[]) ?? [],
        });
      } catch (err) {
  ...
  head: () => ({
    meta: [
      { title: "حكيم AI — أفضل عروض وكوبونات السعودية" },
      {
        name: "description",
        content:
          "منصة سعودية ذكية تجمع أفضل العروض والكوبونات ومقارنة الأسعار في مكان واحد، مرتّبة بالذكاء الاصطناعي.",
      },
      { property: "og:title", content: "حكيم AI — أفضل عروض وكوبونات السعودية" },
      {
        property: "og:description",
        content: "قارن الأسعار واكتشف أعلى نسب التوفير في المتاجر السعودية بالذكاء الاصطناعي.",
      },
...
// ─── مكوّن القسم الحي (memo) ─────────────────────────────
const LiveDataSection = memo(function LiveDataSection({
  prices,
  coupons,
}: {
  prices: LivePrice[];
  ...
  if (prices.length === 0 && coupons.length === 0) return null;
        ...
        <div className="bg-card rounded-3xl p-6 border border-border/60 shadow-card">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xl font-bold text-foreground">
              {t("home.livePricesTitle")}
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {prices.map((item) => {
              const displayPrice =
                item.price_with_tax ?? item.price_before_tax ?? 0;
              const storeName = item.store?.name_ar ?? t("home.defaultStore");
              return (
                <div
                  key={item.id}
                  className="p-4 rounded-2xl bg-secondary/50 border border-border/40 flex justify-between items-center"
                >
                  <div className="min-w-0">
                    <h3 className="font-bold text-sm truncate">
                      {item.products?.name_ar || item.product_id || t("home.product")}
                    </h3>
                    <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1">
                      <Tag className="w-3 h-3 text-primary" /> {storeName}
                    </p>
                  </div>
                  <div className="text-left shrink-0">
                    <span className="font-black text-primary text-base">
                      {displayPrice} {t("home.currency")}
                    </span>
                    <span className="block text-[10px] text-muted-foreground">
                      {t("home.beforeTax")}: {item.price_before_tax} {t("home.currency")}
                    </span>
                  </div>
                </div>
              );
        ...
        <div className="bg-card rounded-3xl p-6 border border-border/60 shadow-card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {coupons.map((coupon) => (
              <div
                key={coupon.id}
                className="p-4 rounded-2xl bg-primary/10 border border-primary/30 flex justify-between items-center"
              >
                <div>
                ...
                <div className="text-left">
                  <span className="text-xs font-bold bg-primary text-primary-foreground px-2.5 py-1 rounded-full">
                    {t("home.discount")} {coupon.discount_value}%
                  </span>
                </div>
              </div>
            ))}
...
function Home() {
  ...
  const top = useMemo(() => bestDeals(6), []);
  const groups = useMemo(() => comparableGroups().slice(0, 3), []);
  ...
  const submitSearch = (e: React.FormEvent) => {
    ...
    const text = q.trim();
    if (!text) return;
    navigate({ to: "/chat", search: { q: text } });
  };

  const runQuickSearch = (term: string) => {
    setQ(term);
    navigate({ to: "/chat", search: { q: term } });
  };
  ...
  const pillars = useMemo(
    () =>
      [
        {
          to: "/deals",
          label: t("pillar.deals"),
          icon: Flame,
          tone: "from-primary/25 to-primary/5",
          note: t("pillar.dealsNote"),
        },
        {
          to: "/",
          label: t("home.compareTitle"),
          icon: TrendingDown,
          tone: "from-accent/25 to-accent/5",
          note: t("pillar.compareNote"),
          hash: "compare",
        },
        {
          to: "/coupons",
          label: t("item.coupons"),
          icon: Ticket,
          tone: "from-primary/25 to-primary/5",
          note: t("pillar.couponsNote"),
        },
        {
          to: "/stores",
          label: t("item.stores"),
          icon: StoreIcon,
          tone: "from-accent/25 to-accent/5",
          note: t("pillar.storesNote"),
        },
        {
          to: "/real-estate",
          label: t("pillar.realEstate"),
          icon: HomeIcon,
          tone: "from-primary/25 to-primary/5",
          note: t("pillar.realEstateNote"),
        },
        {
          to: "/maps",
          label: t("pillar.maps"),
          icon: MapPin,
          tone: "from-primary/25 to-primary/5",
          note: t("pillar.mapsNote"),
        },
        {
          to: "/chat",
          label: t("pillar.assistant"),
          icon: Bot,
          tone: "from-accent/25 to-accent/5",
          note: t("pillar.assistantNote"),
        },
      ] as const,
    [t],
  );
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 pt-6 space-y-14">
      ...
      <section className="relative overflow-hidden rounded-[2.5rem] bg-gradient-hero p-6 md:p-12 shadow-glow">
        ...
        <div className="relative">
          ...
          <h1 className="font-thuluth text-4xl md:text-7xl leading-[1.6] tracking-normal">
            {t("home.title1")}
            <br />
            <span className="drop-shadow-[0_2px_6px_rgba(0,0,0,0.55)]">
              {t("home.title2")}
            </span>
          </h1>
          ...
          <form onSubmit={submitSearch} role="search" className="mt-7 max-w-2xl">
            <div className="flex items-center gap-2 bg-card text-card-foreground rounded-2xl p-2 shadow-glow border border-primary/20">
              ...
              <button
                type="submit"
                className="inline-flex items-center gap-1.5 bg-gradient-gold font-bold px-4 md:px-5 py-2.5 rounded-xl hover:opacity-95 transition"
              >
                <Search className="w-4 h-4" />
                <span className="hidden md:inline leading-normal">
                  {t("home.search")}
                </span>
              </button>
            </div>
            <div className="flex flex-wrap gap-2 mt-3">
              {["أرز بسمتي", "زيت دوار الشمس", "iPhone 16", "عروض النهدي"].map(
                (s) => (
                  <button
                    key={s}
                    type="button"
                    onClick={() => runQuickSearch(s)}
                    className="leading-normal text-[13px] font-semibold px-3 py-1.5 rounded-full bg-black/30 hover:bg-black/45 backdrop-blur border border-white/25"
                  >
                    {s}
                  </button>
                ),
      ...
      {/* الأسعار الحية والكوبونات */}
      <LiveDataSection prices={liveData.prices} coupons={liveData.coupons} />
                  ...
                  >
                    <div className="p-4 bg-gradient-to-l from-primary/10 to-transparent flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {g[0].unit && (
                          <p className="text-xs text-muted-foreground">{g[0].unit}</p>
                        )}
                      </div>
                      <div className="text-left shrink-0">
                        ...
                        return (
                          ...
                          >
                            ...
                            {i === 0 ? (
                              ...
                              <span className="text-xs text-hot font-bold">
                                +{diff} {t("home.currency")}
                              </span>
                            )}
                            <span className="font-display font-black w-16 text-left">
                              {d.price} {t("home.currency")}
                            </span>
                          </div>
                        );
                      })}
      ...
      <LazySection minHeight={420}>
        <section>
          <SectionHeader
            title={t("home.aiToolsTitle")}
            subtitle={t("home.aiToolsSubtitle")}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              {
                to: "/compare",
                tag: t("home.ai.compareTag"),
                title: t("home.ai.compareTitle"),
                desc: t("home.ai.compareDesc"),
                cta: t("home.ai.compareCta"),
              },
              {
                to: "/analysis",
                tag: t("home.ai.analysisTag"),
                title: t("home.ai.analysisTitle"),
                desc: t("home.ai.analysisDesc"),
                cta: t("home.ai.analysisCta"),
              },
              {
                to: "/ads",
                tag: t("home.ai.adsTag"),
                title: t("home.ai.adsTitle"),
                desc: t("home.ai.adsDesc"),
                cta: t("home.ai.adsCta"),
              },
              {
                to: "/market",
                tag: t("home.ai.marketTag"),
                title: t("home.ai.marketTitle"),
                desc: t("home.ai.marketDesc"),
                cta: t("home.ai.marketCta"),
              },
            ].map((c) => (
              <Link
                key={c.to}
                to={c.to}
                className="bg-card rounded-3xl border border-border/60 shadow-card p-5 hover-lift flex flex-col"
              >
                <span className="self-start text-[11px] font-bold px-2.5 py-1 rounded-full bg-primary/10 text-primary border border-primary/30 leading-normal">
                  {c.tag}
                </span>
                ...
                <span className="mt-4 self-start text-sm font-bold text-primary leading-normal">
                  {c.cta} ←
                </span>
              </Link>
            ))}
          ...
          {pillars.map((p) => {
            ...
            return (
              <Link
                key={p.label}
                to={p.to}
                hash={"hash" in p ? (p.hash as string) : undefined}
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${p.tone} opacity-60 group-hover:opacity-100 transition`}
                />
                <div className="relative">
                  ...
                  <div className="font-black text-sm md:text-base leading-normal">
                    {p.label}
                  </div>
      ...
      <LazySection minHeight={220}>
        <section className="grid md:grid-cols-3 gap-4">
          {[
            {
              icon: ShieldCheck,
              t: t("home.feature.trustTitle"),
              d: t("home.feature.trustDesc"),
            },
            {
              icon: Award,
              t: t("home.feature.luxuryTitle"),
              d: t("home.feature.luxuryDesc"),
            },
            {
              icon: Bot,
              t: t("home.feature.assistantTitle"),
              d: t("home.feature.assistantDesc"),
            },
          ].map((f) => (
            <div
              key={f.t}
              className="p-5 rounded-3xl bg-secondary text-secondary-foreground shadow-card"
            >
      ...
      <LazySection minHeight={140}>
        <section className="pb-10">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-bold text-base text-muted-foreground leading-snug">
              {t("home.partnersTitle")}
            </h3>
            <Link
              to="/stores"
              className="text-xs font-bold text-primary flex items-center gap-1 hover:gap-2 transition-all leading-normal"
            >
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4">
            {stores.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className="shrink-0 flex items-center gap-2 bg-card border border-border/60 rounded-2xl px-3 py-2"
                >
                  <div
...
function Stat({ n, l }: { n: string; l: string }) {
  return (
    <div className="bg-black/35 backdrop-blur rounded-2xl p-3 text-center border border-white/20">
      <div className="font-display font-black text-xl md:text-2xl text-primary-foreground">
        {n}
      </div>
      <div className="text-xs md:text-sm font-semibold text-primary-foreground/90">
        {l}
      </div>
    </div>
  );
    ...
    <div className="flex items-end justify-between mb-4">
      <div>
        <div className="flex items-center gap-2 text-primary">
          {icon}
          <h2 className="font-black text-2xl md:text-3xl leading-snug text-foreground">
            {title}
          </h2>
        </div>
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 pt-6 space-y-14">
      ...
      <LazySection minHeight={420}>
        <section>
          <SectionHeader
            title="قدرات الذكاء الاقتصادي"
            subtitle="أدوات ذكية تخدم قرارك الشرائي"
            icon={<Zap className="w-5 h-5" />}
          />
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              {
                to: "/compare",
                tag: "محدث لحظياً",
                title: "محرك مقارنة الأسعار والعروض",
                desc: "قارن سعر نفس المنتج بين نون وأمازون وجرير وإكسترا ومتجر حكيم — وادفع أقل سعر ممكن.",
                cta: "قارن الآن",
              },
              {
                to: "/analysis",
                tag: "تحليل عميق",
                title: "تحليل المتاجر والمنافسين بالذكاء الاصطناعي",
                desc: "تحليل SWOT متكامل: تموضع الأسعار، نقاط القوة والضعف، حساب هامش الربح، واستراتيجيات النمو وتخفيض تكلفة الشحن.",
                cta: "ابدأ التحليل",
              },
              {
                to: "/ads",
                tag: "توليد فوري",
                title: "مولد محتوى الإعلانات والوصف التسويقي",
                desc: "نصوص إعلانية بلهجة سعودية لسناب شات وتيك توك وإنستغرام، مع تحسين الكلمات المفتاحية SEO.",
                cta: "ولّد إعلانك",
              },
              {
                to: "/market",
                tag: "عقارات وتجارة",
                title: "سوق حكيم التجاري والعقاري الموحد",
                desc: "منتجات وعقارات في الرياض وجدة والخبر مع حاسبة العائد الإيجاري، مدى و STC Pay، وشحن سبل وسمسا خلال 24-48 ساعة.",
                cta: "تصفح السوق والحاسبة",
              },
            ].map((c) => (
              <Link
                key={c.to}
  ...
  return (
    <div className="flex items-end justify-between mb-4">
      <div>
        <div className="flex items-center gap-2 text-primary">
          {icon}
          <h2 className="font-black text-2xl md:text-3xl leading-snug text-foreground">{title}</h2>
        </div>
