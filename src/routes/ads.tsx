  ...
  head: () => ({
    meta: [
      { title: "مولد محتوى الإعلانات — حكيم AI" },
      {
        name: "description",
        content:
          "توليد نصوص إعلانية وحملات تسويقية بلهجات سعودية مخصصة لسناب شات وتيك توك وإنستغرام مع تحسين الكلمات المفتاحية SEO.",
      },
      { property: "og:title", content: "مولد محتوى الإعلانات — حكيم AI" },
      {
        property: "og:description",
        content: "نسخ إعلانية جاهزة بلهجة سعودية لكل منصة، مع هاشتاقات وكلمات مفتاحية.",
      },
...
function AdsPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const out = await run({
        data: { product: product.trim(), audience, platform, dialect, tone },
      });
      setResult(out);
    } catch (err) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 pt-6 pb-16 space-y-6">
      <header className="rounded-[2rem] bg-gradient-hero text-primary-foreground p-6 md:p-9 shadow-glow relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="font-display text-2xl md:text-4xl font-black">
            مولد محتوى الإعلانات والوصف التسويقي
          </h1>
          <p className="mt-3 max-w-2xl text-sm md:text-base text-white/80 leading-relaxed">
            توليد نصوص إعلانية وحملات تسويقية بلهجات سعودية مخصصة لسناب شات والتيك توك وإنستغرام، مع
            تحسين الكلمات المفتاحية SEO.
          </p>
        </div>
      </header>
      ...
      <form
        onSubmit={submit}
        className="bg-card rounded-3xl border border-border/60 shadow-card p-5 grid md:grid-cols-2 gap-4"
      >
        <label className="block md:col-span-2">
          <span className="text-xs font-bold text-muted-foreground mb-1.5 block">
            المنتج أو الخدمة
          </span>
          <input
            value={product}
        ...
        <section className="space-y-4">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {result.copies.map((c, i) => (
              <div
                key={i}
                className="bg-card rounded-3xl border border-border/60 shadow-card p-5 flex flex-col"
              >
                ...
                <div className="mt-3 inline-flex self-start text-xs font-bold px-3 py-1.5 rounded-full bg-primary/10 text-primary border border-primary/30">
                  {c.cta}
                </div>
                <button
                  onClick={() => copy(`${c.hook}\n${c.body}\n${c.cta}`)}
                  className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-primary self-start"
                >
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            />
            <TagBox
              title="كلمات SEO المفتاحية"
              items={result.keywords}
              onCopy={() => copy(result.keywords.join("، "))}
            />
          </div>
        </section>
      )}
    ...
    <div className="bg-card rounded-3xl border border-border/60 shadow-card p-5">
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((t, i) => (
          <span
            key={i}
            className="text-xs font-bold px-3 py-1.5 rounded-full bg-secondary/50 border border-border/60"
          >
            {t}
          </span>
        ))}
