...
export const Route = createFileRoute("/_authenticated/merchant-review")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "اعتماد التجّار ومراجعة العروض الحقيقية قبل نشرها في حكيم AI.",
      },
...
function ReviewPage() {
  ...
  const setMerchantStatus = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["staff-merchants"] });
    },
  ...
  const setDealStatus = useMutation({
    ...
    onSuccess: () => {
      toast.success("تم التحديث");
      qc.invalidateQueries({ queryKey: ["staff-merchant-deals"] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
        ...
        <ul className="space-y-2">
          {(dealsQ.data ?? []).map((d) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  setDealStatus.mutate({ id: d.id, status: "rejected", note: "يحتاج تعديل" })
                }
                className="text-destructive"
                title="رفض"
              >
