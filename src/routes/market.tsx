...
import { Building2, ShoppingBag, CreditCard, Truck, Sparkles, Calculator, MapPin } from "lucide-react";
...
export const Route = createFileRoute("/market")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "تصفح المنتجات والعقارات في السعودية (الرياض، جدة، الخبر) مع حاسبة العائد الإيجاري، مدى و STC Pay، والشحن السريع عبر سبل وسمسا." },
      { property: "og:title", content: "سوق حكيم التجاري والعقاري الموحد" },
      { property: "og:description", content: "منتجات وعقارات في مكان واحد مع حاسبة العائد الإيجاري ودفع آمن وشحن سريع." },
...
function MarketPage() {
  ...
  const listings = useMemo(
    () => realEstateListings.filter((l) => city === "الكل" || String(l.city ?? "").includes(city)).slice(0, 9),
    [city],
  );
    ...
    <main className="max-w-6xl mx-auto px-4 pt-6 pb-16 space-y-6">
      <header className="rounded-[2rem] bg-gradient-hero text-primary-foreground p-6 md:p-9 shadow-glow relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="font-display text-2xl md:text-4xl font-black">سوق حكيم التجاري والعقاري الموحد</h1>
          <p className="mt-3 max-w-2xl text-sm md:text-base text-white/80 leading-relaxed">
            تصفح المنتجات والعقارات في السعودية (الرياض، جدة، الخبر) مع حاسبة العائد الإيجاري، ربط مدى و STC Pay والشحن السريع عبر سبل وسمسا.
          </p>
        </div>
      </header>

      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <Trust icon={<CreditCard className="w-4 h-4" />} title="مدى و STC Pay" sub="دفع آمن وسريع" />
        <Trust icon={<Truck className="w-4 h-4" />} title="سبل وسمسا" sub="توصيل خلال 24-48 س" />
        <Trust icon={<Sparkles className="w-4 h-4" />} title="ذكاء Gemini 3.6" sub="مقارنات وتحليلات دقيقة" />
        <Trust icon={<ShoppingBag className="w-4 h-4" />} title="بالريال السعودي SAR" sub="شامل ضريبة القيمة المضافة" />
      </div>

      <div className="flex items-center gap-2">
        <TabBtn active={tab === "products"} onClick={() => setTab("products")} icon={<ShoppingBag className="w-4 h-4" />} label="منتجات" />
        <TabBtn active={tab === "estate"} onClick={() => setTab("estate")} icon={<Building2 className="w-4 h-4" />} label="عقارات" />
      </div>
      ...
      {tab === "products" ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {priceProducts.map((p) => {
            ...
            return (
              <article key={p.id} className="bg-card rounded-3xl border border-border/60 shadow-card p-5 hover-lift">
                <div className="text-[11px] font-bold text-primary mb-1">{p.category}</div>
                <h2 className="font-bold leading-snug">{p.name}</h2>
                <p className="text-xs text-muted-foreground mt-1">{p.brand}{p.unit ? ` · ${p.unit}` : ""}</p>
                <div className="mt-4 flex items-end justify-between">
                  <div>
                    <div className="text-[10px] text-muted-foreground">أفضل سعر لدى {r.name}</div>
                    <div className="font-display font-black text-xl text-gold-shine">{finalPrice(best)} ر.س</div>
                  </div>
                  <Link to="/compare" className="text-xs font-bold px-3 py-2 rounded-xl bg-primary/10 text-primary border border-primary/30">
                    قارن الأسعار
                  </Link>
                </div>
              </article>
        ...
        <>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {listings.map((l) => (
              <article key={l.id} className="bg-card rounded-3xl border border-border/60 shadow-card overflow-hidden hover-lift">
                {l.image && (
                  <img src={l.image} alt={l.title} loading="lazy" decoding="async" className="w-full h-40 object-cover" />
                )}
                <div className="p-5">
                  <h2 className="font-bold leading-snug line-clamp-1">{l.title}</h2>
                  <p className="text-xs text-muted-foreground mt-1 inline-flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-primary" /> {l.district ?? ""}{l.city ? ` · ${l.city}` : ""}
                  </p>
            ...
            {listings.length === 0 && <p className="text-sm text-muted-foreground">لا يوجد حالياً عقار في هذه المدينة.</p>}
          </div>
        </>
      )}
...
function Trust({ icon, title, sub }: { icon: React.ReactNode; title: string; sub: string }) {
  return (
    <div className="bg-card rounded-2xl border border-border/60 p-4 flex items-center gap-3">
      <span className="w-9 h-9 rounded-xl bg-primary/10 text-primary grid place-items-center shrink-0">{icon}</span>
