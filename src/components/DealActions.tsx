  ...
  async function handleAlert() {
    ...
    try {
      await upsertAlert({
        data: {
          dealId: deal.id,
          productKey: deal.productKey ?? null,
          title: deal.title,
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="p-4 rounded-2xl border border-primary/20 bg-card space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleAlert}
              disabled={busy === "alert"}
              className="mr-auto px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-bold press-ripple disabled:opacity-50"
            >
      ...
      {showCash && (
        <div className="p-4 rounded-2xl border border-primary/20 bg-card space-y-3">
          <p className="text-sm text-muted-foreground">
            سجّل مبلغ شرائك من <b>{deal.storeId}</b> — كاش باك <b className="text-primary">2%</b>{" "}
            يُضاف لمحفظتك بعد المراجعة.
          </p>
          <div className="flex items-center gap-2">
            <input
              type="number"
