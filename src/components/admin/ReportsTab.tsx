...
export function ReportsTab() {
  ...
  const settingsQ = useQuery({
    ...
    queryFn: () => listReportSettings(),
  });
  const previewQ = useQuery({
    queryKey: ["report-preview", days],
    queryFn: () => previewReport({ data: { days } }),
  });
  ...
  const addM = useMutation({
    ...
    onSuccess: () => {
      setEmail("");
      setName("");
      toast.success("تمت إضافة المستلم");
      invalidate();
    },
  ...
  const sendM = useMutation({
    mutationFn: (test: boolean) =>
      sendReportNow({ data: { days, testEmail: test ? testEmail : undefined } }),
    onSuccess: (r) => {
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-primary/20 bg-card/60 p-5">
        ...
        <p className="text-sm text-muted-foreground">
          يُرسل تلقائيًا كل يوم أحد الساعة 6 صباحًا ويتضمن ملخص الأداء والتغيّر مقارنة بالفترة
          السابقة المماثلة.
        </p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          >
            <RefreshCw className={`w-4 h-4 ${previewQ.isFetching ? "animate-spin" : ""}`} /> تحديث
            المعاينة
          </button>
        </div>
      </div>
        ...
        {previewQ.isLoading || !summary ? (
          ...
          <>
            <p className="text-xs text-muted-foreground mb-3">
              {fmtDate(summary.periodStart)} — {fmtDate(summary.periodEnd)} • مقارنة بـ{" "}
              {fmtDate(summary.prevStart)} — {fmtDate(summary.prevEnd)}
            </p>
                ...
                return (
                  <div key={m.key} className="rounded-xl border border-border/60 p-3">
                    ...
                    <div className="text-[11px] text-muted-foreground mt-1">
                      السابق: {m.previous.toLocaleString("ar-SA")}
                    </div>
                  </div>
                );
              })}
                  ...
                  {summary.topDeals.map((d, i) => (
                    ...
                    >
                      ...
                      <span className="text-muted-foreground whitespace-nowrap">
                        {d.clicks} نقرة • {d.conversions} تحويل
                      </span>
                    </li>
                  ))}
                </ul>
      ...
      <div className="rounded-2xl border border-border bg-card/60 p-5">
        ...
        {settingsQ.data?.recipients.length ? (
          ...
          <p className="text-sm text-muted-foreground">
            لا يوجد مستلمون بعد — أضف بريدًا لبدء استلام التقرير.
          </p>
        )}
        ...
        <div className="mt-5 flex flex-wrap gap-2 items-center border-t border-border/50 pt-4">
          ...
          <button
            ...
            disabled={!testEmail || sendM.isPending}
          ...
          <ul className="space-y-2 text-sm">
            {settingsQ.data.runs.map((run) => {
              ...
              return (
                <li
                  key={run.id}
                  className="flex flex-wrap items-center gap-2 border-b border-border/40 py-2"
                >
                  <span className={`text-xs px-2 py-0.5 rounded-md border ${s.cls}`}>
                    {s.label}
                  </span>
                  <span>
