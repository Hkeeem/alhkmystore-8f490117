...
export function NoonSyncSettingsPanel() {
  ...
  const couponsMutation = useMutation({
    mutationFn: async () => {
      ...
      return coupons.filter((c) => (c.storeName ?? "").includes("نون") || c.storeId === "noon")
        .length;
    },
    onSuccess: (count) => {
      ...
      void queryClient.invalidateQueries({ queryKey: REAL_DEALS_KEY });
      toast.success(
        count > 0 ? `تم جلب ${count} كوبون من نون` : "لا توجد كوبونات نون فعّالة حاليًا",
      );
    },
        ...
        <CardTitle className="flex items-center gap-2 text-lg">
          ...
          {status?.linked ? (
            <Badge variant="default" className="gap-1">
              <BadgeCheck className="size-3" /> مربوطة فعليًا
            </Badge>
          ) : status?.configured ? (
            <Badge variant="secondary">المعرّف محفوظ</Badge>
          ) : (
            <Badge variant="outline" className="gap-1">
              <XCircle className="size-3" /> غير مهيّأة
            </Badge>
          )}
        </CardTitle>
      </CardHeader>
        ...
        {statusQuery.isLoading ? (
          ...
          <>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                <RefreshCw
                  className={`size-4 ${couponsMutation.isPending ? "animate-spin" : ""}`}
                  aria-hidden="true"
                />
              ...
              {!status?.configured && (
                <p className="text-xs text-muted-foreground">
                  احفظ معرّف الناشر أولًا حتى تعمل المزامنة بالربط الرسمي.
                </p>
              )}
            </div>
